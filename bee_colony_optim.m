...
dominio=[-3 3];                     % Domínino da primeira variável de projeto
dominio(2,:) = [-3 3];  % Domínino da segunda variável de projeto
tol = 1e-6;     % Tolerância da diferença da f(n-1) e f(n)     
n_abelhas = 100;   % Nº de operárias na colméia
n_variaveis = 2;  % Variáveis de projeto
n_seguidoras = 10; % Número de seguidoras da colméia
iter_final = 50;    % Número de iterações com f(n-1)-f(n) < tol
...
figure(2);
fsurf(@(x,y)  100*(y-x.^2).^2+(1-x).^2); hold on;
scatter3(operarias_novas(1,1),operarias_novas(1,2),f(operarias_novas(1,:)));
disp(['Iterações: ',num2str(iter)]);
disp(' ');
disp(['Valor da f: ',num2str(min(armazenaf))]);

xlim([dominio(1,1) dominio(1,2)]);
ylim([dominio(2,1) dominio(2,2)]);